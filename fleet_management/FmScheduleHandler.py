...
class FmScheduleHandler():
    ...
    def track_idle_time(self, r_id, last_node_id, home_dock_loc_ids):
        """
    ...
    def manage_robot(self, f_id, r_id, m_id=None, v_id=None):
        ...
        # ------------------------------------------------------------------
        self.track_idle_time(r_id, last_node_id, home_dock_loc_ids)
        ...
        # ------------------------------------------------------------------
        # Update Horizon Visualization State
        # ------------------------------------------------------------------
        # The manage_traffic call populates temp_fb variables with the robot's current states.
        # We store these active horizons to uniquely paint the terminal graph.

        # Update base visualization dicts
        self.traffic_handler.task_handler.visualization_handler.robot_positions[r_id] = \
            {'x': current_position[0], 'y': current_position[1]}

        if not hasattr(self.traffic_handler.task_handler.visualization_handler, 'active_horizons'):
            self.traffic_handler.task_handler.visualization_handler.active_horizons = {}
        ...
        if vis_horizon:
            self.traffic_handler.task_handler.visualization_handler.active_horizons[r_id] = {
                'horizon': vis_horizon,
                'horizon_release': vis_release
            }
        else:
            # Robot is idle / task complete — remove its entry so coloured
    ...
    def _handle_iteration_interval(self, f_id, r_id, m_id, v_id, cleared,
        ...
        # 2. finished a task but it is a loop task.
            ...
            if last_node_id in charge_dock_loc_ids:
    ...
    def cancel_task(self, f_id, r_id, m_id, v_id):
        ...
        if success:
            ...
            if not target_dock and home_dock_loc_ids:
