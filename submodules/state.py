...
class StateSubscriber:
    ...
    def __init__(self, fleetname, versions, dbconn, mqttclient=None,
        ...
        # Eliminates the O(N) fetch_all_data DB scan in fetch_mex_data.
    ...
    def process_message(self, msg):
        ...
        try:
            ...
            # Update in-memory cache before DB write — cache is always fresh.
    ...
    def fetch_data(self, f_id, r_id, m_id):
        ...
        cached_msg = self.cache.get(r_id)
        if cached_msg and cached_msg.get("zoneSetId") == f_id and cached_msg.get("manufacturer") == m_id:
            # Extract values from cached message
            serial_number = cached_msg.get("serialNumber")
            maps = cached_msg.get("maps")
            order_id = cached_msg.get("orderId")
            last_node_id = cached_msg.get("lastNodeId")
            driving = cached_msg.get("driving")
            paused = cached_msg.get("paused")
            node_states = cached_msg.get("nodeStates")
            agv_position = cached_msg.get("agvPosition")
            velocity = cached_msg.get("velocity")
            battery_state = cached_msg.get("batteryState")
            errors = cached_msg.get("errors")
            information = cached_msg.get("information")
        ...
        try:
            ...
            if result:
                ...
                maps = json.loads(result[6]) if isinstance(result[6], str) else result[6] 
